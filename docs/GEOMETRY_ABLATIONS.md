...
`codex/speed-radius-cap` 从半径分支 `c9387af` 继续开发，移植固定中心速度实现，新增 S1 和同分支 A/B/C/D 四组对照。实现/审计提交 `06a4390`，70 项相关测试通过，未训练。实测速度项在新规则下使单框 mask 改变率从 0.218% 升至 1.023%，合并图改变率从 4.318% 升至 17.959%。详细条件、逐类结果及命令见 [速度/半径组合消融](SPEED_RADIUS_ABLATION.md)。

下方为此前两个独立分支的交付记录；S1 没有合入 C1 中心移动。
