...
| B1-M1 | run_id：`20260902_211006`<br>分支：`codex/matching-gt-nearest`<br>commit：`4c96ba6` | 基于旧 B1<br>匹配改为 GT-nearest + proposal reuse<br>其余 q、mask、归一化和训练参数不变<br>batch：16×16 | `/mnt/nas_data/guqiupeng/checkpoint_nes/b1_teacher_value_no_scale_20260902_211006/last.ckpt` | mAP **0.3895** / NDS **0.5003**<br>mATE 0.6268 / mASE 0.2663 / mAOE 0.4311 / mAVE 0.4010 / mAAE 0.2193 |
| B1T-M1 | run_id：`20260902_210959`<br>分支：`codex/matching-gt-nearest`<br>commit：`4c96ba6` | 基于旧 B1T<br>匹配改为 GT-nearest + proposal reuse<br>elliptical mask 与其余参数不变<br>batch：16×16 | `/mnt/nas_data/guqiupeng/checkpoint_nes/b1t_teacher_value_elliptical_mask_20260902_210959/last.ckpt` | mAP **0.3909** / NDS **0.5023**<br>mATE 0.6215 / mASE 0.2606 / mAOE 0.4375 / mAVE 0.3882 / mAAE 0.2240 |
| B2-M1 | run_id：`20260902_210640`<br>分支：`codex/matching-gt-nearest`<br>commit：`7d4fd24` | 基于旧 B2<br>匹配改为 GT-nearest + proposal reuse<br>AdaptiveGTScalerV3 与其余参数不变<br>batch：16×16 | `/mnt/nas_data/guqiupeng/checkpoint_nes/b2_teacher_value_adaptive_scale_20260902_210640/last.ckpt` | mAP **0.3877** / NDS **0.5015**<br>mATE 0.6371 / mASE 0.2623 / mAOE 0.4182 / mAVE 0.3928 / mAAE 0.2129 |

当前结论：旧匹配下的 B2 仍是单次最优。M1 使 B1/B1T 的 mAP 小幅上升但 NDS 下降，并使 B2 的 mAP/NDS 同时下降；当前不支持用 M1 替换旧匹配。所有结果只有 seed 0。
