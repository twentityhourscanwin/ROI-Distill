...
def build_experiment(bundle, *, model_cls=LabelDistill,
    ...
    if config.region.scaler.type == 'velocity_only_half':
        scaler = VelocityOnlyHalfScaler(
            displacement_fraction=config.region.scaler.displacement_fraction,
            past_time_seconds=config.region.scaler.past_time_seconds,
            future_time_seconds=config.region.scaler.future_time_seconds,
            center_mode=config.region.scaler.center_mode,
        )
    else:
