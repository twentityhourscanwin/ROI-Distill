...
class NuscDetDataset(Dataset):
    ...
    def __init__(self,
        ...
        if isinstance(info_paths, list):
            ...
            for info_path in info_paths:
                self.infos.extend(mmengine.load(info_path))
        else:
            self.infos = mmengine.load(info_paths)
        ...
        self.max_distance = 58
        self.max_lidar_points_num = 320000
    ...
    def sample_lidar_augmentation(self):
        """Generate bda augmentation values based on bda_config."""
        # if self.is_train:
        #     lidar_idx = np.random.choice(9, 7, replace=False)
        # else:
    ...
    def get_lidar_points(self, lidar_infos, bda_rot, lidar_idx):
        keyego2global_trans = torch.tensor(-np.array(lidar_infos[0][0]['LIDAR_TOP']['ego_pose']['translation']))
        keyego2global_rot = torch.tensor(
            Quaternion(lidar_infos[0][0]['LIDAR_TOP']['ego_pose']['rotation']).rotation_matrix,
            dtype=torch.float32).T

        sensor2ego_trans = torch.tensor(np.array(lidar_infos[0][0]['LIDAR_TOP']['calibrated_sensor']['translation']))
        sensor2ego_rot = torch.tensor(
            Quaternion(lidar_infos[0][0]['LIDAR_TOP']['calibrated_sensor']['rotation']).rotation_matrix,
            dtype=torch.float32)

        sweep_ptss_list = list()
        for lidar_info_sweeps in lidar_infos:
            sweep_ptss = list()
            ts = lidar_info_sweeps[0]['LIDAR_TOP']['timestamp'] / 1e6
            for idx, lidar_info in enumerate(lidar_info_sweeps):
                if idx not in lidar_idx:
                    continue
                lidar_path = lidar_info['LIDAR_TOP']['filename']
                points = np.fromfile(os.path.join(self.data_root,
                                                  lidar_path),
                                     dtype=np.float32,
                                     count=-1).reshape(-1, 5)
                sweep_ts = lidar_info['LIDAR_TOP']['timestamp'] / 1e6
                points[:, 4] = ts - sweep_ts

                sweepglobal2ego_trans = torch.tensor(np.array(lidar_info['LIDAR_TOP']['ego_pose']['translation']))
                sweepglobal2ego_rot = torch.tensor(
                    Quaternion(lidar_info['LIDAR_TOP']['ego_pose']['rotation']).rotation_matrix,
                    dtype=torch.float32)
                ...
                # Transform sensor to ego frame
                points[:, :3] = (sensor2ego_rot[:3, :3] @ points[:, :3].unsqueeze(-1)).squeeze(-1)
                points[:, :3] += sensor2ego_trans[:3]

                # Transform key global to sweep ego frame
                points[:, :3] = (sweepglobal2ego_rot[:3, :3] @ points[:, :3].unsqueeze(-1)).squeeze(-1)
                points[:, :3] += sweepglobal2ego_trans[:3]

                # Transform ego to key global frame
            ...
            if num_points > self.max_lidar_points_num:
                choices = np.random.choice(num_points, self.max_lidar_points_num, replace=False)
                sweep_ptss = sweep_ptss[choices]
            else:
                append = self.max_lidar_points_num - num_points
                sweep_ptss = torch.cat([sweep_ptss, -999 * torch.ones(append, num_feat)], dim=0)
            sweep_ptss_list.append(sweep_ptss)

        return torch.stack(sweep_ptss_list)
    ...
    def get_image(self, cam_infos, cams, lidar_infos=None):
        ...
        # for ind, c in enumerate(cameras):
        #     image_file = os.path.join(self.data_root, cam_infos[0][c]['filename'])
...
def collate_fn(data,
    ...
    for iter_data in data:
        ...
        if is_return_lidar:
            lidar_points = iter_data[10]
            lidar_points_batch.append(lidar_points)
        if is_return_depth:
    ...
    if is_return_lidar:
        ret_list.append(torch.stack(lidar_points_batch))
    if is_return_depth:
