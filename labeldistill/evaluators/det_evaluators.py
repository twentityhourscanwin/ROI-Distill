...
class DetNuscEvaluator():
    ...
    def _format_bbox(self, results, img_metas, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmengine.track_iter_progress(results)):
            ...
            ########################################################################
            #for lidar pretrained
            # ego2global_translation = np.array(img_metas[sample_id]['ego2global_translation'])
            # ego2global_rotation = Quaternion(img_metas[sample_id]['ego2global_rotation'])
            #
            # sensor2ego_translation = np.array(img_metas[sample_id]['sensor2ego_translation'])
            # sensor2ego_rotation = Quaternion(img_metas[sample_id]['sensor2ego_rotation'])
            ########################################################################
            ...
            for i, box in enumerate(boxes):
                ...
                ########################################################################
                # lidar coord
                # nusc_box.rotate(sensor2ego_rotation)
                # nusc_box.translate(sensor2ego_translation)
                #
                # nusc_box.rotate(ego2global_rotation)
                # nusc_box.translate(ego2global_translation)
                ########################################################################
