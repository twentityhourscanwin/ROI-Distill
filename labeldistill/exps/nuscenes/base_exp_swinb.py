...
H = 900
W = 1600
final_dim = (640, 1600)  # 修改：Swin-B输入尺寸
...
backbone_conf = {
    ...
    'img_backbone_conf': dict(
        type='SwinTransformer',  # 修改：使用Swin Transformer
        embed_dims=128,
        depths=[2, 2, 18, 2],
        num_heads=[4, 8, 16, 32],
        window_size=7,
    ...
    'img_neck_conf': dict(
        type='SECONDFPN',
        in_channels=[256, 512, 1024],  # 修改：Swin-B的输出通道 (stage 2,3,4)
        upsample_strides=[0.5, 1, 2],  # 修改：对应的上采样步长
...
class LabelDistillModel(LightningModule):
    ...
    def __init__(self,
        ...
        # 修改：train_info_paths 改为包含 train 和 val 数据集
