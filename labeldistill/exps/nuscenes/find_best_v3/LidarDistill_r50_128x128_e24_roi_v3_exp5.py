# v3_exp5: wider spread + larger mu: w_l=0.3, w_h=0.8, mu=0.20
# ROI-Guided Feature Distillation v3: 统一比例系数 + 三级分层权重
# 相比 v2 的核心改动:
#   1. Mask: QualityAwareMaskGeneratorV3 — 三级权重区间 [0,w_l],[w_l,w_h],[w_h,1]
#      高质量: k = w_h + (1-w_h)*s_roi  中等: k = w_l + (w_h-w_l)*s_roi
#      未匹配: k = w_l*(1-r/R_max)^2
#   2. Scale: AdaptiveGTScalerV3 — μ * l_gt 作为统一放缩上限
#      高质量: Δl = min(|Δx_local|, μ*l_gt) + f(v)
#      中等: Δl = min(d_x, μ*l_gt) + f(v)
#      未匹配: Δl = (1-r/R_max)^2 * f(v)
...
class LabelDistillModel(BaseLabelDistillModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.generate_bev_mask = QualityAwareMaskGeneratorV3(
            w_l=0.3, w_h=0.8, r_max=50.0, boost_small_medium=True)
        self.proposal_target_layer = ProposalTargetLayer()
        self.change_gt = AdaptiveGTScalerV3(
            mu=0.2, s_vel=0.2, r_max=50.0)
...
if __name__ == '__main__':
    run_cli(LabelDistillModel,
            'LidarDistill_r50_128x128_e24_roi_v3_exp5',
