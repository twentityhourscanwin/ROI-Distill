#labeldistill/exps/nuscenes/labeldistill/Lidar_gt_label_convnextb_900x1600_e30.py
# ConvNeXt-B 900x1600 + 简单GT蒸馏策略（无ROI/FP，无通道分割）
# 作为 test 提交的中间baseline：BEVDepth → [本实验] → ROI+FP完整版
# 使用 LabelDistillFull（全通道送入adaptor，不做通道切分）
...
from labeldistill.models.lidardistill_full import LabelDistillFull
...
class LabelDistillModel(BaseLabelDistillModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.model = LabelDistillFull(self.backbone_conf,
                                      self.head_conf,
                                      self.lidar_conf,
                                      lidar_ckpt_path,
                                      is_train_depth=True)
    ...
    def training_step(self, batch):
        ...
        response_loss = 0.6 * response_loss

        if len(depth_labels.shape) == 5:
            depth_labels = depth_labels[:, 0, ...]
        depth_loss = self.get_depth_loss(depth_labels.cuda(), depth_preds, sigma=1.5)

        lidar_distill_loss = self.get_feature_distill_loss(
            lidar_feats, distill_feats_lidar, targets[0], binary_mask=False) * 0.6
        ...
        self.log('train/response_loss',    response_loss,      on_step=True, prog_bar=False, sync_dist=True)
        self.log('train/distill_loss',     lidar_distill_loss, on_step=True, prog_bar=False, sync_dist=True)
...
if __name__ == '__main__':
    run_cli(LabelDistillModel,
            'Lidar_gt_label_convnextb_900x1600_e30',
