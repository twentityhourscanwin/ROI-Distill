...
class BEVDepthHead(CenterHead):
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d):
        ...
        Override CenterHead.get_targets to accept (gt_bboxes_3d, gt_labels_3d)
        as two separate lists (one per sample in the batch), matching the
        call convention used by BaseBEVDepth and base_exp scripts.

        Args:
            gt_bboxes_3d (list[torch.Tensor]): Ground truth boxes per sample.
            gt_labels_3d (list[torch.Tensor]): Labels per sample.

        Returns:
            tuple(list[torch.Tensor]): (heatmaps, anno_boxes, inds, masks)
        """
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    new_idx = k
                    x, y = center_int[0], center_int[1] #x: horizontal(downward), y: vertical(left2right)
