...
class KDHead(CenterHead):
    ...
    def __init__(
        ...
        self.trunk = MODELS_DET.build(bev_backbone_conf)   # ResNet 注册在 mmdet
        self.trunk.init_weights()
        self.neck = MODELS_3D.build(bev_neck_conf)          # SECONDFPN 注册在 mmdet3d
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d):
            ...
            Returns:
                tuple[list[torch.Tensor]]: Tuple of target including
                    ...
                    - list[torch.Tensor]: Masks indicating which
                        boxes are valid.
        """
        heatmaps, anno_boxes, inds, masks, bev_mask, bev_box, bev_label = multi_apply(
        ...
        #stack label input and bev mask
        bev_box = torch.stack(bev_box, 0)
        bev_label = torch.stack(bev_label, 0)
        bev_mask = torch.stack(bev_mask, 0)
        return bev_mask, bev_box, bev_label, (heatmaps, anno_boxes, inds, masks)
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        for idx, mask in enumerate(task_masks):
            ...
            for m in mask:
                ...
                label_class.append(gt_labels_3d[m])
            ...
            label_classes.append(
                torch.cat(label_class).long().to(gt_bboxes_3d.device))
            flag2 += len(mask)
        draw_gaussian = draw_heatmap_gaussian
        heatmaps, anno_boxes, inds, masks = [], [], [], []
        # bev_mask = gt_bboxes_3d.new_zeros(
        #     (feature_map_size[0], feature_map_size[1]),
        #     dtype=torch.uint8,
        #     device='cuda')
        bev_label = gt_labels_3d.new_zeros(
            (feature_map_size[0], feature_map_size[1], 10),
            dtype=torch.float32,
            device='cuda')
        bev_box = gt_labels_3d.new_zeros(
            (feature_map_size[0], feature_map_size[1], 9),
            dtype=torch.float32,
            device='cuda')

        bev_mask = np.zeros((feature_map_size[0], feature_map_size[1]))
        # bev_label = np.zeros((feature_map_size[0], feature_map_size[1], 19))
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    bev_mask, bev_box, bev_label = gen_labelinput(bev_mask,
                                                                 bev_box,
                                                                 bev_label,
                                                                 center,
                                                                 width,
                    ...
                    new_idx = k
                    x, y = center_int[0], center_int[1] #x: vertical(downward), y: horizontal(left2right)
        ...
        bev_mask = torch.tensor(bev_mask,
                                dtype=torch.uint8,
                                device='cuda')
        return heatmaps, anno_boxes, inds, masks, bev_mask, bev_box, bev_label
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            cls_avg_factor = torch.clamp(reduce_mean(
                heatmaps[task_id].new_tensor(num_pos)),
                                         min=1).item()
            loss_heatmap = self.loss_cls(preds_dict[0]['heatmap'].type(torch.float32),
                                         heatmaps[task_id],
    ...
    def response_loss(self, targets, preds_dicts, teacher_dicts, **kwargs):
        ...
        for task_id, (preds_dict, teacher_dict) in enumerate(zip(preds_dicts, teacher_dicts)):
            ...
            teacher_heatmap = clip_sigmoid(
                teacher_dict[0]['heatmap']).detach()

            num_pos = heatmaps[task_id].eq(1).float().sum().item()
            cls_avg_factor = torch.clamp(reduce_mean(
                heatmaps[task_id].new_tensor(num_pos)),
                                         min=1).item()
            loss_heatmap = self.loss_cls(student_heatmap.float(),
                                         heatmaps[task_id],
                                         avg_factor=cls_avg_factor)
