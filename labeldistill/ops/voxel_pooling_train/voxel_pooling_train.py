...
class VoxelPoolingTrain(Function):
    ...
    def forward(ctx, geom_xyz: torch.Tensor, input_features: torch.Tensor,
        ...
        # 显存优化：不立即分配 grad_input_features，只保存 shape 和 dtype
        # 在 backward 时才分配，减少 forward 阶段的峰值显存
        ctx.input_features_shape = input_features.shape
        ctx.input_features_dtype = input_features.dtype
        ctx.input_features_device = input_features.device
        ...
        # save pos_memo for backward (grad_input_features 延迟分配)
        ctx.save_for_backward(pos_memo)
        return output_features.permute(0, 3, 1, 2)

    @staticmethod
    def backward(ctx, grad_output_features):
        (pos_memo,) = ctx.saved_tensors
        # 显存优化：延迟分配 grad_input_features，只在 backward 时分配
        # 这样可以避免 forward 阶段就占用 2.26GB 显存
        grad_input_features = torch.zeros(
            ctx.input_features_shape,
            dtype=ctx.input_features_dtype,
            device=ctx.input_features_device
        )
        kept = (pos_memo != -1)[..., 0]
