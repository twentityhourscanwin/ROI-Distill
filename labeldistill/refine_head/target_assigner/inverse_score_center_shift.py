"""Blend Proposal and GT centers using complementary proposal-score weights."""
...
class InverseScoreCenterShift:
    """Weight Proposal by ``1-score`` and GT by ``score``."""
    ...
    def forward(self, result: MatchResult) -> MatchResult:
        ...
        for boxes, proposals, matched, effective, scores in zip(
            ...
            if selected.any():
                ...
                gt_weight = score[:, None]
                shifted[selected, :2] = (
                    (1 - gt_weight) * proposal_xy + gt_weight * gt_xy)
